...
void AFileSend::BeginPlay()
{
	Super::BeginPlay();
	
}

void AFileSend::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

void AFileSend::SendFileToServer(const FString& ServerIP, int32 ServerPort, const FString& FilePath)
{
	//파일 로드
	...
	//파일 정보 전송 (ex: "VOICE,filename.wav,1024")
	{
	...
	//파일 데이터 전송
	{
		...
		while (BytesSent < TotalBytes)
		{
			...
			// 서버 응답 대기
			FString Response = ReceiveString();
			if (Response.Contains(TEXT("CONTINUE"))) // 계속 전송
			{
				continue;
			}
			...
			else if (Response.Contains(TEXT("RETRANSMIT"))) // 마지막 청크 재전송
			{
